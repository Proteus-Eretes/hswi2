...
import { resolveFiles, defineNuxtModule, addPlugin, addTemplate, isNuxt3, resolveAlias, addWebpackPlugin, addVitePlugin, addPluginTemplate, useNuxt, nuxtCtx, addComponent, installModule, loadNuxtConfig, normalizeTemplate, compileTemplate, tryResolvePath, normalizePlugin, templateUtils } from '@nuxt/kit';
...
var SegmentParserState;
(function(SegmentParserState2) {
  ...
  SegmentParserState2[SegmentParserState2["catchall"] = 3] = "catchall";
})(SegmentParserState || (SegmentParserState = {}));
var SegmentTokenType;
(function(SegmentTokenType2) {
  SegmentTokenType2[SegmentTokenType2["static"] = 0] = "static";
  SegmentTokenType2[SegmentTokenType2["dynamic"] = 1] = "dynamic";
  SegmentTokenType2[SegmentTokenType2["catchall"] = 2] = "catchall";
})(SegmentTokenType || (SegmentTokenType = {}));
  ...
  for (const file of files) {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (child) {
        ...
        if (isLastSegment && tokens.length === 1 && tokens[0].type === 1) {
          route.path += "?";
        }
      }
    }
...
function getRoutePath(tokens) {
  return tokens.reduce((path, token) => {
    return path + (token.type === 1 ? `:${token.value}` : token.type === 2 ? `:${token.value}(.*)*` : encodePath(token.value));
  }, "/");
}
  ...
  return {
    ...
    transform(code) {
      ...
      for (const regex of excludeRE) {
        Array.from(withoutComment.matchAll(regex)).flatMap((i) => [
          ...i[1]?.split(seperatorRE) || [],
          ...i[2]?.split(seperatorRE) || []
        ]).map((i) => i.replace(importAsRE, "").trim()).filter(Boolean).forEach((i) => matched.delete(i));
      }
      if (!matched.size) {
        return null;
      }
...
const version = "3.0.0-27282980.f011a60";
